...
let Recorder = function (stream, config) {
  ...
  let recorder = createScript.apply(context, [4096, 1, 1]);// 单声道采样
  ...
  let audioData = {
    ...
    //  for (let j = 0; j < this.buffer.length; j++) {
      //  for (let k = 0; k < this.buffer[j].length; k++) {
          //  let a = Math.max(-1, Math.min(1, this.buffer[j][k]));
          //  console.log(a);
      //  }
    //  }
  ...
  this.upload = function (url, callback) {
    ...
    if (callback) {
      ...
      xhr.addEventListener('bort', function (e) {
        // callback('cancel', e);
      }, false);
    }
    xhr.open('POST', url);
...
Recorder.get = function (callback, config) {
  if (callback) {
    if (navigator.getUserMedia) {
      ...
      Recorder.throwErr('当前浏览器不支持录音功能。');
    }
  }
};
